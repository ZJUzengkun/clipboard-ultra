...
- **收藏板（Pinboards）** — 顶部 BoardBar 分组管理，星标即归板；板成员永不过期
- **卡片内容编辑** — 文本条目支持浮层编辑（铅笔按钮 / 选中后按 `E`），标签与归属保留
- **使用频率统计** — 重复复制/粘贴累加次数，卡片底部展示「用过 N 次」
- **来源应用记录** — 记录内容来自哪个应用，卡片底部显示来源名
- **数据导入 / 导出** — 一键导出为自包含 JSON（含图片、标签规则、收藏板），导入按内容去重合并
- **正则标签筛选** — 基于正则表达式自动打标签，支持按标签快速筛选
- **手动打标** — 用户可对任意条目手动编辑/覆盖标签
- **内置检测器** — JSON / URL / 银行卡（Luhn）/ 身份证（加权校验）等校验算法识别
- **图片剪贴板** — 支持图片复制记录（本地 blob 存储 + 缩略图预览）
- **按内容类型过期策略** — 图片/文字/其他类型独立配置保留天数 + 最大保存数量上限
- **卡片差异化配色** — 仿 Paste 风格，标签/类型/内容决定 Header 颜色（8 色轮选）；两行头部布局
- **快捷删除与多级撤销** — Backspace 删除，Cmd+Z 多级撤销（延迟落库）
- **快捷键直选** — `⌘/Ctrl+1~9` 直接粘贴前 9 张卡片
- **全局快捷键** — `Ctrl+Shift+V` / `Cmd+Shift+V` 快速唤起
- **多显示器光标跟随** — 面板在光标所在屏幕底部弹出
- **多主题** — 深色/浅色 + 多套配色，跟随系统或手动切换
- **开机自启 & 权限自检** — 可配置登录启动；macOS 辅助功能权限状态检测与引导
- **自动更新** — 内置检查更新，一键下载安装并重启（Tauri updater）
- **系统托盘** — 后台常驻，右键菜单操作
- **独立设置窗口** — 可配置快捷键、外观、标签规则、隐私、数据管理、更新
...
- [ ] 确认 macOS 自动粘贴（CGEvent）实机长期稳定
- [ ] 敏感内容防护（疑似密码/token 默认打码）
- [ ] 快速预览（空格键查看长文本 / 大图）
- [ ] 真实来源 App 图标（从 bundle 提取）
- [ ] 富文本支持（HTML/RTF）
- [ ] OCR 识别插件（从图片中提取文字）
- [ ] 多语言支持
- [ ] 剪贴板同步（跨设备）
...
| 版本 | 主要变更 |
|------|----------|
| v0.9.2 | 使用频率统计、来源应用记录、卡片内容编辑、数据导入/导出、卡片放大与 Paste 风格头部、多显示器光标跟随定位 |
| v0.9.x | 开机自启、权限自检、呼出聚焦搜索框可配置 |
| v0.8.9 | 收藏板（Pinboards）：顶部 BoardBar + 星标归板交互 |
